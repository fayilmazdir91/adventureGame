...
public abstract class BattleLoc extends Location {
	...
	public boolean onLocation() {
		...
		System.out.println("Hey, be careful and sure of being well-prepared. Here is " + this.getObstacle().getName()
				+ "'s place!!!");
		if (!whatToDo()) {
			return false;
		}
	...
	public boolean whatToDo() {
		...
		default:
			System.out.println(
					"Mr. " + this.getPlayer().getUsername() + ", please respecify what you would like to do...");
			whatToDo();
			break;
		}
	...
	public boolean combat() {
		...
		System.out.println("Nooo!!! " + numObstacle + " " + this.getObstacle().getName() + "s are present in the "
				+ this.getName());
		...
		for (int i = 0; i < numObstacle; i++) {
			while (0 < this.getPlayer().getHealth() && 0 < this.getObstacle().getHealth()) {
				...
				if (this.getObstacle().getHealth() <= 0) {
					System.out.println("Your Health: " + this.getPlayer().getHealth() + " "
							+ this.getObstacle().getName() + "'s Health: 0");
				} else {
					if (this.getPlayer().getHealth() <= 0) {
						System.out.println("Your Health: 0 " + this.getObstacle().getName() + "'s Health: "
								+ this.getObstacle().getHealth());
					} else {
						System.out.println("Your Health: " + this.getPlayer().getHealth() + " "
								+ this.getObstacle().getName() + "'s Health: " + this.getObstacle().getHealth());
					}
			...
			if (this.getObstacle().getHealth() <= 0) {
				...
				setWealthLoc();
				System.out.println("Your Health: " + this.getPlayer().getHealth() + " Your Wealth: "
						+ this.getPlayer().getWealth());
				System.out.println();
		...
		System.out.println("Character: " + this.getPlayer().getName() + "\tWeapon: " + this.getPlayer().getWeaponName()
				...
				+ "\t\tInventory: food-" + (this.getPlayer().getInventory().getFood()) + " water-"
				+ (this.getPlayer().getInventory().getWater()) + " firewood-"
				+ (this.getPlayer().getInventory().getFirewood()) + "Bonus Armor: "
				+ (this.getPlayer().getInventory().getBonusArmor()));
	...
	public void setWealthLoc() {
		if (this.getObstacle().getId() == 4) { // ID: 4
			...
			if (chanceMajor < 0.15) { // chance of WEAPON
				...
				System.out
						.println("OPPPS... Nothing has come out. " + "You're pretty much lucky, right...? HAH HAH HAA");
			}
